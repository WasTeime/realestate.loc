...
use common\components\{exceptions\ModelSaveException, helpers\UserFileHelper, helpers\UserUrl};
use Exception;
use Throwable;
use Yii;
...
class ExportImportBehavior extends Behavior
{
    ...
    /**
     * Атрибут модели для даты создания
     */
    public string $createdAtAttribute;
    ...
    {
        if (isset($this->uniqueField) && (!isset($this->createdAtAttribute, $this->updatedAtAttribute))) {
            throw new InvalidConfigException(
                'If model is unique `createdAtAttribute` and `updatedAtAttribute` must be defined'
            );
        }
    ...
    final public function export(): ?array
    {
        ...
        //$mainItem = $class::find()->where([$class::primaryKey()[0] => $model->primaryKey])->asArray()->one();
        $mainItem = [];
     ...
     * @throws StaleObjectException
     */
    private function _deleteModel(array $data, ExportImportBehavior|BaseActiveRecord $model, string $class, bool $createLog): void
    {
     ...
     * @throws Exception
     */
    private function _importModel(array $data, ExportImportBehavior|BaseActiveRecord $model, string $class, bool $createLog): void
    {
        if (isset($this->uniqueField) && $model->{$this->uniqueField} && $createLog) {
            ModelImportLog::add($class, $this->uniqueField, $model->{$this->uniqueField}, $model->export(), $data);
        }
