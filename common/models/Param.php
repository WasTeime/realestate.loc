...
class Param extends AppActiveRecord
{
    ...
    {
        if ($group) {
            $items = self::find()->where(['group' => $group, 'is_active' => Boolean::Yes->value])->asArray()->all();
        } else {
            $items = self::find()->where(['is_active' => Boolean::Yes->value])->asArray()->all();
        }
        $result = [];
        foreach ($items as $item) {
            ...
            try {
                ...
                $value = match (ParamType::from($item['type'])) {
                    ParamType::Image, ParamType::File => file_exists($filename) && !is_dir($filename)
    ...
    final public function getColumnValue(): bool|array|string|null
    {
        try {
            ...
            $value = match (ParamType::from($this->type)) {
                ParamType::Image => !empty($this->value) && file_exists($filename) && !is_dir($filename)
                    ? Html::img(
                        UserUrl::toAbsolute($this->value),
                        ['style' => 'max-width:150px;max-height:50px;', 'alt' => '']
                    ) : null,
                ParamType::File => !empty($this->value) && file_exists($filename) && !is_dir($filename)
                    ? Html::a(
                        basename($this->value),
                        UserUrl::toAbsolute($this->value),
                        ['target' => '_blank']
