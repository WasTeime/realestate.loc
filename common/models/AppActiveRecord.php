...
abstract class AppActiveRecord extends ActiveRecord
{
    ...
    /**
     * Список полей с координатами
     */
    protected array $_pointFields = [];
     ...
     * @return static
     */
    public static function findOne($condition, bool $force = false): ?AppActiveRecord
    {
        static $_cache = [];
        $param = self::conditionToString($condition);
        if ($force) {
            return $_cache[$param] = parent::findOne($condition);
        }
        return $_cache[$param] ?? $_cache[$param] = parent::findOne($condition);
    }
     ...
     * @return static[]
     */
    public static function findAll($condition, bool $force = false): array
    {
        static $_cache = [];
        $param = self::conditionToString($condition);
        if ($force) {
            return $_cache[$param] = parent::findAll($condition);
        }
        return $_cache[$param] ?? $_cache[$param] = parent::findAll($condition);
    }
    ...
    public function __get($name)
    {
        ...
        if (is_null($value) && in_array($name, $this->attributes(), true) && str_contains($name, '.')) {
            $data = explode('.', $name);
    ...
    /**
     * Создание уникальной строки из условия поиска
     */
    private static function conditionToString(float|array|int|string|Expression $condition, string $prefix = ''): string
    {
        $string = sprintf('__%s_%s', $prefix, str_replace(['{{%', '}}'], '', static::tableName()));
        if ($condition instanceof Expression) {
            $string .= '_' . $condition->expression;
        } elseif (is_array($condition) && count($condition) > 0) {
            $string .= self::implodeConditionArrayRecursive($condition);
    ...
    /**
     * Рекурсивное объединение значений массива в единую строку
     */
    private static function implodeConditionArrayRecursive(array $cond): string
    {
        $string = '';
        foreach ($cond as $key => $value) {
            $string .= '_' . $key;
            if (is_array($value)) {
                $string .= self::implodeConditionArrayRecursive($value);
            } else {
                $string .= '_' . $value;
            }
        }
    ...
    {
        foreach ($this->_pointFields as $field => $attr) {
