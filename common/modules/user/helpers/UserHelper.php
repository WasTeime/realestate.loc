...
class UserHelper
{
     ...
     * @throws \yii\base\Exception
     */
    public static function createNewUserByEmail(string $username, string $password): User
    {
        $user = new User();
    ...
    public static function createUserEmail(
        ...
        $email_model = $userModule->emailModel;
        if (!$user_email = $email_model::findOne(['user_id' => $user->id])) {
            $user_email = new $email_model();
        ...
        if (!$userModule->enableEmailVerification ||
            ...
            try {
                EmailHelper::sendVerificationEmail($user_email, $user);
            } catch (Exception $e) {
                UserFileHelper::saveDataToFile(
                    [
                        'message' => $e->getMessage(),
                        'file' => $e->getFile(),
                        'line' => $e->getLine()
                    ],
                    'VerificationEmailError_' . date('d.m.Y H:i')
                );
            }
        }
     ...
     * @throws ModelSaveException
     */
    public static function createUserExt(ActiveRecord|IdentityInterface|User $user, Boolean $rules_accepted = Boolean::No): UserExt
    {
        $userExt = new UserExt();
