...
use common\modules\user\{helpers\EmailHelper, helpers\UserHelper, models\User, Module};
...
#[RequestFormData(
    properties: [
        ...
        new OA\Property(property: 'first_name', description: 'Имя', type: 'string'),
        new OA\Property(property: 'middle_name', description: 'Отчество', type: 'string'),
        new OA\Property(property: 'last_name', description: 'Фамилия', type: 'string'),
        new OA\Property(property: 'phone', description: 'Телефон', type: 'string')
    ]
)]
        ...
        switch ($field) {
            case 'email':
                ...
                if ($this->sendMail === true) {
                    EmailHelper::sendVerificationEmail($email, $user);
                }
                if (!$email->save()) {
